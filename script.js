...
(() => {
  ...
  // Minimal built-in list; replace words.txt for a bigger set
  const BUILTIN_WORDS = [
    "about","other","which","there","their","would","could","after","first","those","again","every","think","three","small",
    "place","great","right","still","world","house","under","never","water","point","woman","young","story","money","maybe",
    "music","short","light","north","south","sound","night","heart","happy","brown","black","white","green","array","sweet",
    "grown","child","adult","truly","touch","found","build","bring","guess","quite","peace","rough","crown","crowd","glass",
    "plant","train","plane","chair","table","apple","bread","cheer","smile","laugh","tears","angry","quiet","storm","rainy",
    "windy","sunny","clear","cloud","ocean","river","beach","mount","plain","valley","field","stone","brick","steel","motor",
    "cable","video","graph","photo","frame","pixel","sound","voice","anger","pride","honor","fancy","spice","sugar","lemon",
    "toast","spoon","knife","forks","plate","baker","cocoa","cream","cheek","sweep","bloom","flame","blaze","shine","shade",
    "chalk","paper","piano","flute","guitar","drums","actor","stage","scene","laugh","dream","sleep","woken","early","later",
    "maybe","shall","ought","kinda","trick","solve","puzzle","rhyme","catch","truly","spare","alone","along","among","ahead",
    "aside","above","below","draft","final","print","share","reset","start","endow","enter","press","click","touch","swipe",
    "close","open","spilt","spill","smell","taste","scent","flour","earth","metal","brick","dairy","sheep","goats","cows",
    "camel","zebra","tiger","otter","whale","shark","eagle","panda","rhino","corgi","hound","horse","raven","robin","doves",
    "geese","spoon","waltz","jazzy","fuzzy","pique","quilt","quart","quark","oxide","ionic","civic","kayak","level","madam",
    "radar","refer","sagas","solos","stats","tenet","minim","xylem","glyph","nymph","jumpy","vivid","zesty","azure","khaki"
  ].filter(w => w.length === 5);
  ...
  let locks = [];  // tile state locks after reveal
  ...
  function saveSettings() { ls('wordish:settings', settings); }
  function saveStats() { ls('wordish:stats', stats); }
    ...
    if (settings.wordSource === "words.txt") {
      try {
        ...
        if (list.length >= 50) {
          ...
          showToast("words.txt too small — using built‑in list.");
        }
      } catch (e) {
  ...
  function isHardModeSatisfied(guess) {
    ...
    // Must keep greens in position and include all known yellows
    const prevRow = row-1;
    for (let c=0;c<5;c++) {
      if (locks[prevRow][c] === "correct" && guess[c] !== grid[prevRow][c]) return false;
    }
    // Collect required letters from previous row (present+correct), ensure they appear
    const required = [];
  ...
  function scoreGuess(guess) {
    // Two-pass: greens, then yellows using counts
  ...
  function endGame(win) {
    ...
    if (win) {
      ...
      stats.lastWin = new Date().toISOString();
    } else {
      stats.streak = 0;
    }
    saveStats();
  ...
  function submitGuess() {
    ...
    if (win) return setTimeout(() => { showToast("Nice!"); endGame(true); }, 5*260+100);

    row++;
    col = 0;
    if (row >= settings.guessLimit) {
      setTimeout(() => { showToast(`It was “${solution.toUpperCase()}”`); endGame(false); }, 5*260+100);
    }
  }
